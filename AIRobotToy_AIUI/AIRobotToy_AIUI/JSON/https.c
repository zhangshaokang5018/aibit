...
//          printf("socket reuse: %d \n", sock_fd);
    ...
    len = snprintf(request, 1024,
            ...
            "X-Platform: iOS\r\n"
            "X-Biz-Id: xftjapp\r\n"
            "X-Client-Version: 2.0.1199\r\n"
            "X-Session-Id: 512199876020347890\r\n"
            "Content-Length: %d\r\n"
            "%s\r\n"
            "%s",
            dir, host, port,
...
/*---------------------------------------------------------------------*/
int http_post_binary(HTTP_INFO *hi, char *url, char *data, int data_len, char *response, int size)
{
    char        request[20480], err[100];
    char        host[256], port[10], dir[1024];
    int         sock_fd, https, verify, send_len = data_len;
    int         ret, opt, len;
    socklen_t   slen;
...
//          printf("socket reuse: %d \n", sock_fd);
    ...
    memcpy(&request[len], data, send_len);
    // strcpy(&request[len], "test");
    // memcpy(&request[len], "test", 5);
    len += send_len;
    printf("request:%s\n", request);
...
int http_write_header(HTTP_INFO *hi)
{
    ...
    if (hi->request.xChannel[0] != 0) {
        len += snprintf(&request[len], H_FIELD_SIZE,
                        "%s\r\n", hi->request.xChannel);
    }
    if (hi->request.xPlatform[0] != 0) {
        len += snprintf(&request[len], H_FIELD_SIZE,
                        "%s\r\n", hi->request.xPlatform);
    }
    if (hi->request.xBizId[0] != 0) {
        len += snprintf(&request[len], H_FIELD_SIZE,
                        "%s\r\n", hi->request.xBizId);
    }
    if (hi->request.xClientVersion[0] != 0) {
        len += snprintf(&request[len], H_FIELD_SIZE,
                        "%s\r\n", hi->request.xClientVersion);
    }
    if (hi->request.xSessionId[0] != 0) {
        len += snprintf(&request[len], H_FIELD_SIZE,
                        "%s\r\n", hi->request.xSessionId);
    }
